...
Construct your Components to build a Single Page Application. Keep your components separate and design them as if they are pages before adding in your Router.
 ...
 XConstruct an AXIOS request to POST to add a Smurf to the Smurf DB you'll need all three fields.

 XCreate a form that will allow users to add Smurfs to the Smurf DB.

 XIf a Smurf is created correctly, you should see a response that is an array of Smurfs with unique id's assigned to each Smurf.
...
Add a Router to this application by using React Router.
 ...
 XThen make it so you can navigate to your routes using Link.

 x Create two routes in your App component, one at '/' for your Smurfs component,and one at /smurf-form for your form.

 XThen in your App component, create a nav bar that will use NavLink to route to your different pages.
